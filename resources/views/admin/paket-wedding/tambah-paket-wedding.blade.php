@extends('admin/template-admin')
@section('konten')
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Tambah Paket Wedding</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Dashboard v1</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
    <!-- /.content-header -->

    <!-- Main content -->
    <section class="content">
      <div class="container-fluid">
        <div class="card card-body">
            <div class="mb-3">
                <a href="/admin/paket-wedding/data-paket" class="btn btn-primary">Lihat Data</a>
            </div>
            <form action="/admin/paket-wedding/simpan-paket" method="post" enctype="multipart/form-data">
                @csrf
                <label for="">Nama Paket</label>
                <input type="text" class="form-control" name='nama_paket' required>
                <br>
                <label for="">Harga </label>
                <input type="number" class="form-control" name='harga_paket' required>
                <br>
                <label for="">Deskripsi</label>
                <textarea name="deskripsi" id="" class="form-control" required></textarea>
                <br>
                <label for="">Foto </label>
                <input type="file" class="form-control" name='foto_paket' required>
                <button type="submit" class="btn btn-primary">Simpan</button>
                <button type="reset" class="btn btn-secondary">Reset</button>
                <br>
            </form>
        </div>
      </div>
    </section>
    <!-- /.content -->
  </div>
  <!-- /.content-wrapper -->
@endsection